...
docker service create --name ${SERVICE[2]} --replicas 1 --network ${NET} \
--constraint 'node.hostname!=jack' \
armhf/memcached:alpine
}

function createNextCloud {
  echo "Creating Memcache service ${SERVICES[3]} with volumes ${VOLUMES[1]}, ${VOLUMES[2]}, ${VOLUMES[3]}, and ${VOLUMES[4]}"
...
--env DB_HOST=${SERVICES[0]} \
--env REDIS_HOST=${SERVICES[1]} \
--env MEMCACHE_HOST=${SERVICE[2]} \
